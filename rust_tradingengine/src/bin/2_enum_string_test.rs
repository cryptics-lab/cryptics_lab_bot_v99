...
#[derive(Debug, Clone, Serialize, Deserialize)]
pub struct SimpleOrderString {
    pub order_id: String,
    pub direction: Direction,
}

async fn enum_string_test() -> Result<()> {
    // Configuration
    let schema_registry_url = "http://localhost:8081";
    let bootstrap_servers = "localhost:9092";
    let topic = "test.enum.string.topic";
    
    println!("=== ENUM AS STRING TEST ===");
    
    // Define a simple schema with direction as string instead of enum
    let schema_str = r#"
    {
      "type": "record",
      "name": "SimpleOrderString",
    ...
    println!("1. Registering schema with registry (using string type for enum)...");
    ...
    // Create test data
    let simple_order = SimpleOrderString {
        order_id: "TEST12345-STRING".to_string(),
    ...
    // Encode data
    println!("3. Encoding data with schema registry (as string)...");
    ...
    let consumer: StreamConsumer = ClientConfig::new()
        .set("group.id", "enum-string-test-group")
        .set("bootstrap.servers", bootstrap_servers)
    ...
    println!("
    {{
      \"name\": \"test-enum-string-sink\",
      \"config\": {{
        \"connector.class\": \"io.confluent.connect.jdbc.JdbcSinkConnector\",
        \"tasks.max\": \"1\",
        \"topics\": \"test.enum.string.topic\",
